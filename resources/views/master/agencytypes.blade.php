@extends('layouts.app')
@section('top')
<h1 class="h2">Settings - Agency Types</h1>
@endsection
@section('content')
<div class="card">
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <select class="custom-select" placeholder="Choose type of document" id="doc_type" name="doc_type" required>
            ...
            @foreach($agencytype as $at)
            <option value="{{ $at->agency_type_id }}" > {{ $at->agency_type }} </option>
            @endforeach
        </select>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">New Agency Type Details</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
$('#editdt').on('click', function() {
  ...
  var id = $('#doc_type').val();
     ...
     $.ajax ({
      url : '../settings/getagencytypeinfo?id='+id
      ,method : 'GET'
      ,cache : false
      ,beforeSend:function() {
      ...
      $('#did').val(jsondata['agency_type_id']);
      $('#atype').val(jsondata['agency_type']);
...
$('#btnsaveedit').on('click', function() {
  ...
  if(confirm('Are you sure you want to save your changes?')) {
      ...
      $.ajax ({
        url : '../settings/agency-types/update?'+thisform
        ,method : 'GET'
        ,cache : false
        ,beforeSend:function() {
        //$('#loadModal').modal({ backdrop: 'static' });
      ...
      window.open('../settings/agency-types','_self');
      }).fail( function(response) {
...
$('#btnadd').on('click', function() {
  ...
  if(confirm('Are you sure you want to save your changes?')) {
      ...
      $.ajax ({
        url : '../settings/agency-types/add?'+thisform
        ,method : 'GET'
        ,cache : false
        ,beforeSend:function() {
        //$('#loadModal').modal({ backdrop: 'static' });
...
$('#deldt').on('click', function() {
  ...
  if(confirm('Are you sure you want to delete this record?')) {
      ...
      $.ajax ({
        url : '../settings/agency-types/delete?id='+id
        ,method : 'GET'
        ,cache : false
        ,beforeSend:function() {
        //$('#loadModal').modal({ backdrop: 'static' });
